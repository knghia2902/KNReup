...
<objective>
Mở rộng OmniVoice engine và FastAPI routes để hỗ trợ đầy đủ: clone giọng (zero-shot), voice design (tạo giọng từ mô tả text), preview giọng, và quản lý profile (CRUD + metadata). Đây là backend foundation cho toàn bộ Voice Clone tool.
</objective>
...
1. **`voice_design(description, text, output_path)`** — Tạo giọng từ mô tả text (không cần audio ref):
   ```python
...
6. **`get_audio_duration(audio_path)`** — Trả về thời lượng audio:
   ```python
...
3. **POST `/tts/profiles/preview`** — Preview giọng clone:
   ```python
...
4. **GET `/tts/profiles/{name}`** — Chi tiết 1 profile:
   ```python
   @router.get("/{name}")
...
- Voice design API (description → generate → save profile)
